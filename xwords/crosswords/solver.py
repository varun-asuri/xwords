...
w, h, ROTATE180, LOCATIONS, ROWS, COLS, START_DICT, START_DICT_INV, CHECK_ALLOWED, PARTIAL_LOOKUP, ALL_WORDS = None, None, None, None, None, None, {}, {}, {}, {}, {}
currdepth = 0
global COUNT
COUNT = 0
...
def set_partial_lookup(bucket, dictionary):
    global PARTIAL_LOOKUP
    letters = dictionary.characters
    for word_len in bucket:
        print("WORD LEN: " + str(word_len))
        words = bucket[word_len]
        for letter in letters:
            for idx in range(word_len):
                PARTIAL_LOOKUP[(idx, word_len, letter)] = set()
        for word in words:
            for idx, letter in enumerate(word):
                PARTIAL_LOOKUP[(idx, word_len, letter)].add(word)
...
#                print("WORD IN USED")
            ...
            if prev_placed is not None:
                ...
                words = [w for w in words if w in PARTIAL_LOOKUP[(idx, length, letter)]]
...
bestdepth = 0
def place_words(board, poss, used, optional_words, last_placed=None, depth=0):
    ...
    heuristics = []
    for k in poss:
        contains_optional = len(poss[k]) > 0 and poss[k][0] in optional_words
        heuristics.append((not contains_optional, len(poss[k]), k))
#    heuristics = sorted(heuristics)
#    maxkey = heuristics[0][2]
#    if heuristics[0][0]:
#        options = [heuristics[i][2] for i in range(min(2, len(heuristics)))]
#        maxkey = random.choice(options)
    maxkey = min(heuristics)[2]
    loc, dir = maxkey
    words = poss[maxkey]
    del poss[maxkey]
    ...
    for word in words:
        ...
        newboard = place_words(board, poss, used | {word}, optional_words - {word}, (maxkey, word), depth + 1)
...
def setup_start(board):
    set_start_lookup(board)
    needed_lengths = {num_direction(board, key[0], key[1]) + 1 for key in START_DICT}
    return needed_lengths


def setup_poss(board, words):
